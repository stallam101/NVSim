...
class SierpinskiTester:
    ...
    def __init__(self, distribution_type="normal"):
        self.nvsim_executable = "./build/nvsim"
        self.distribution_type = distribution_type
        
        # Select configuration files based on distribution type
        if distribution_type == "negative-binomial":
            self.base_config = "configs/sierpinski_test_negative_binomial.cfg"
            self.cell_file = "configs/sierpinski_rram_negative_binomial.cell"
        elif distribution_type == "gamma":
            self.base_config = "configs/sierpinski_test_gamma.cfg"
            self.cell_file = "configs/sierpinski_rram_gamma.cell"
        else:  # normal distribution (default)
            self.base_config = "configs/sierpinski_test.cfg"
            self.cell_file = "configs/sierpinski_rram.cell"
            
        self.p_matrix_file = "measurements/P_matrix.csv"
        self.b_vector_file = "measurements/b_vector.csv"
        self.results_dir = Path("work")
    ...
    def run_nvsim_multiple(self, config_file: str, num_runs: int = 40, timeout: int = 30) -> Dict:
        """Run NVSim multiple times and return minimum latency for noise reduction."""
        all_measurements = []
        successful_runs = 0
...
class SierpinskiDataManager:
    ...
    def __init__(self, output_dir="measurements/sierpinski_data"):
